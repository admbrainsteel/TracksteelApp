...
export const Viewer3DToolbar: React.FC<Viewer3DToolbarProps> = ({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 p-3 bg-slate-900/90 backdrop-blur-xl border border-slate-800 rounded-2xl shadow-2xl text-slate-200">
      {/* Left: Opacity Slider */}
      <div className="flex items-center gap-3 bg-slate-950/60 px-4 py-2 rounded-xl border border-slate-800">
        <Eye className="w-4 h-4 text-amber-400" />
        <span className="text-xs font-medium text-slate-300">Opacidade</span>
        <input
          type="range"
          min={10}
          max={100}
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 bg-slate-950/60 hover:bg-cyan-500 hover:text-slate-950 text-cyan-400 border border-slate-800 hover:border-cyan-400 rounded-xl text-xs font-mono font-bold uppercase tracking-wider transition-all"
          title="Tela Cheia"
        >
          <Maximize2 className="w-3.5 h-3.5" />
